...
docker build ./node -t <container registry>/node-windows-servercore:ltsc2019

docker push <container registry>/node-windows-servercore:ltsc2019

# Build node application
docker build ./app -t <container registry>/hello-arc-windows:latest

docker push <container registry>/hello-arc-windows:latest

# Test locally
docker run -d -p 8080:8080 <container registry>/hello-arc-windows:latest
...
kubectl run hello-arc-linux --image <container registry>/hello-arc --overrides='{"apiVersion": "v1", "spec": {"nodeSelector": { "kubernetes.io/os": "linux" }}}'

# Verify app is working
kubectl port-forward hello-arc-linux 8080:8080

# Test Windows image on AKS Windows node
kubectl run hello-arc-windows --image <container registry>/hello-arc-windows:latest --overrides='{"apiVersion": "v1", "spec": {"nodeSelector": { "kubernetes.io/os": "windows" }}}'
